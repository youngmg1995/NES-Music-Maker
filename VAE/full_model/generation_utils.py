...
def generate_seprsco(tracks, int2labels_maps):
    '''
    ...
    ----------
    tracks : numpy arrays
        The output sequence(s) generated by our VAE.
    int2labels_maps : list of numpy array
        Thse are the inverse mappings used to de-vectorize our sequence.
    ...
    '''
    # Grab # of tracks and features voices per track
    m = len(tracks)
    n = tracks[0].shape[0]
    
    # Flatten each track to single array
    flat_tracks = [tracks[i].reshape((n,-1)) for i in range(m)]
    
    # Devectorize track using int2labels_map
    flat_tracks = [int2labels_maps[i][flat_tracks[i]] for i in range(m)]
    ...
    for i in range(n):
        score = np.zeros((flat_tracks[0].shape[1],m), np.uint8)
        for j in range(m):
            score[:,j] = flat_tracks[j][i]
